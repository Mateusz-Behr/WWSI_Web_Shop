...
namespace Web_Shop_3.Tests_InMemoryDB
{
    public class SqliteInMemoryCustomerRepositoryTest : IDisposable
    {
        ...
        public async Task CustomerRepository_EmailExistsAsync_ReturnsTrue()
        {
            ...
            var emailExists = await customerRepository.EmailExistsAsync("ma.be@gmail.com");
        ...
        public async Task CustomerRepository_IsEmailEditAllowedAsync_ReturnsTrue()
        {
            ...
            var emailEditAllowed = await customerRepository.IsEmailEditAllowedAsync("ma.be@wp.pl", 1);
        ...
        public async Task CustomerRepository_GetByEmailAsync_Exists()
        {
            ...
            var customer = await customerRepository.GetByEmailAsync("ma.be@gmail.com");

            Assert.Equal("Mateusz", customer.Name);
            Assert.Equal("Behr", customer.Surname);
        }
